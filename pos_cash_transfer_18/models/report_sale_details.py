...
class ReportSaleDetails(models.AbstractModel):
    ...
    def _get_taxes_info(self, taxes):
        ...
        if not taxes:
            # Si no hay impuestos, devolvemos un dict con la estructura que el core espera
            return[] 

        for tax_id in taxes:
            if not isinstance(taxes[tax_id], dict):
                continue
            if 'base_amount' not in taxes[tax_id]:
                taxes[tax_id]['base_amount'] = 0.0
        
        # Llamamos al original
        return super()._get_taxes_info(taxes)
    ...
    def get_sale_details(self, date_start=False, date_stop=False, config_ids=False, session_ids=False):
        ...
        for order in orders:
            ...
            for line in order.lines:
                if 'base_amount' not in taxes:
                    taxes['base_amount'] = 0.0 # O inicializar según necesites
        ...
        if payment_ids:
            self.env.cr.execute("""
                SELECT method.id as id, payment.session_id as session, COALESCE(method.name->>%s, method.name->>'en_US') as name, (method.type = 'cash') as cash,
                     sum(amount) total, method.journal_id journal_id
                FROM pos_payment AS payment,
                     pos_payment_method AS method
                WHERE payment.payment_method_id = method.id
                    AND payment.id IN %s
                GROUP BY method.name, method.type, payment.session_id, method.id, journal_id
                ...
                if payment['session'] == session.id:
                    if not payment['cash']:
                        ...
                        for transfer in cash_transfers:
                            cash_in_out_list.append({
                                'name': transfer.memo or transfer.name, # CORREGIDO: memo en lugar de ref
