# -*- coding: utf-8 -*-
"""
Description: Serial Driver for Camiones devices in Odoo IoTBox
Author: [Your Name]
Date: 2024-XX-XX
Version: 0.1
"""
...
BasculaCamiones = namedtuple('BasculaCamiones', SerialProtocol._fields + ('zeroCommand', 'tareCommand', 'clearCommand', 'autoResetWeight'))

CamionesSerialProtocol = BasculaCamiones(
    name='Bascula Camiones',
    baudrate=9600,  
...
class CamionesSerialDriver(ScaleDriver):
    """Driver para dispositivos seriales Camiones en Odoo IoTBox."""
    _protocol = CamionesSerialProtocol

    def __init__(self, identifier, device):
        super(CamionesSerialDriver, self).__init__(identifier, device)
        self.device_manufacturer = 'Camiones Device'
        _logger.info('Iniciando driver para dispositivo Camiones con identificador: %s', identifier)

    @classmethod
    def supported(cls, device):
        """Verifica si el dispositivo conectado es compatible con el protocolo Camiones."""
