...
class ReportSaleDetails(models.AbstractModel):
    ...
    def get_sale_details(self, date_start=False, date_stop=False, config_ids=False, session_ids=False):
        _logger.warning('Get sale details')
        ...
        if (session_ids):
            ...
            domain = AND([domain,[('date_order', '>=', fields.Datetime.to_string(date_start)),
        ...
        for session in sessions:
            ...
            # ====================================================================
            # BLOQUE 1: CUANDO HAY VENTAS EN LA SESIÓN
            # ====================================================================
            for payment in payments:
                ...
                if payment['session'] == session.id:
                    if not payment['cash']:
                        ...
                        net_transfer_out = 0.0
                        for transfer in cash_transfers:
                            # Ignoramos la transferencia de cierre para la matemática y la vista
                            if 'Transferencia saliente a:' in (transfer.ref or ''):
                                continue
                            
                            if transfer.payment_type == 'outbound':
                                net_transfer_out += transfer.amount
                            else:
                                net_transfer_out -= transfer.amount
                                
                            cash_in_out_list.append({
                                'name': transfer.ref,
                                'amount': transfer.amount if transfer.payment_type == 'inbound' else -transfer.amount,
                            })
                            
                        # Fórmula corregida usando el balance inicial real de la sesión
                        payment['final_count'] = payment['total'] + session.cash_register_balance_start + session.cash_real_transaction - net_transfer_out
            ...
            # ====================================================================
            # BLOQUE 2: CUANDO HAY 0 VENTAS EN LA SESIÓN
            # ====================================================================
            if not is_cash_method:
                ...
                # Fórmula corregida usando el balance inicial real de la sesión
                final_count = session.cash_register_balance_start + session.cash_real_transaction - net_transfer_out
                cash_difference = session.cash_register_balance_end_real - final_count
                
                if cash_difference != 0 and cash_moves:
