        ...
        for session in self:
            ...
            if cash_payment_method:
                ...
                cash_transfers = self.env['account.payment'].search([
                    ...
                    ('journal_id', '=', session.cash_journal_id.id), # Solo pagos de esta caja
                ])
                ...
                # Neto de salidas (Si hubo más entradas, será negativo, lo cual sumará al balance final)
                ...
                if session.state == 'closed':
                    session.cash_register_total_entry_encoding = session.cash_real_transaction + total_cash_payment - net_transfer_out - amount_kept
                else:
                    session.cash_register_total_entry_encoding = (
                        sum(session.statement_line_ids.mapped('amount')) + total_cash_payment - net_transfer_out - amount_kept
                    )
    ...
    def try_cash_in_out(self, _type, amount, reason, extras):
        ...
        for session in sessions:
            ...
            try:
                ...
                raise UserError(_("Invalid destination company ID."))
            ...
            # --- LÓGICA DE IN/OUT BLINDADA ---
            # Convertimos a string por si llega como booleano (False) o None desde JS
            safe_type = str(_type).strip().lower()
            is_cash_in = (safe_type == 'in')
            
            # Si es entrada ('in'), el POS recibe (inbound) y la sucursal envía (outbound)
            origin_payment_type = 'inbound' if is_cash_in else 'outbound'
            dest_payment_type = 'outbound' if is_cash_in else 'inbound'
            
            origin_ref = f"{session.name} - {'Entrada desde' if is_cash_in else 'Salida a'} Proveedor: {reason}"
            dest_ref = f"Transferencia a proveedores {'saliente' if is_cash_in else 'entrante'} de: {self.env.company.name} - {reason}"

            # Log para debug en consola de Python
            _logger.info(f"--- DEBUG TYPE --- Recibido: {_type} | Parseado: {safe_type} | is_cash_in: {is_cash_in}")

            # 1. Crear el pago en la compañía de origen (POS actual)
            payment_origin = self.env['account.payment'].create({
                ...
                'payment_type': origin_payment_type, # <-- DINÁMICO
                'pos_session_id': session.id,
                'is_internal_transfer': True,
            })
            payment_origin.action_post()
    
            # 2. Crear la transferencia en la compañía de destino usando with_company
            ...
            payment_dest = self.env['account.payment'].sudo().with_company(destination_company).create({
                ...
                'payment_type': dest_payment_type, # <-- DINÁMICO
                'is_internal_transfer': True,
                'pos_session_id': session.id,
